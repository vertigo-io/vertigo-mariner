...
public class BTTest {
	...
	private static BTNode dispatch(final State state) {
		...
		return selector(
				weather(state)
						.guardedBy(state.bb.eq("i/name", "W")),
				game(state)
						.guardedBy(state.bb.eq("i/name", "G")),
				ticket(state)
						.guardedBy(state.bb.eq("i/name", "T")));
	}
	...
	private static BTNode ticket(final State state) {
		return sequence(
				...
				loopUntil(state.bb.eq("t/idx", "{{t/count}}"),
						sequence(
								state.bb.inc("t/idx"),
	...
	private static BTNode game(final State state) {
		return sequence(
				...
				state.bb.set("g/target",
						Double.valueOf(Math.floor(Math.random() * 101)).intValue()),
				loopUntil(state.bb.eq("g/target", "{{g/choice}}"),
						sequence(
								state.bb.clear("g/choice"),
								state.fulfill("g/choice", "What is your choice ?"),
								state.bb.inc("g/rounds"),
								selector(
										state.display("select down !")
												.guardedBy(state.bb.gt("g/target", "{{g/choice}}")),
										state.display("select up !")
												.guardedBy(state.bb.lt("g/target", "{{g/choice}}")),
										BTNode.succeed()))),
